...
import yugioh.action.{ActionModule, ExistsInAChainAction}
...
trait Resolution {
  ...
  def resolve(existsInAChainAction: ExistsInAChainAction)
             (implicit gameState: GameState, eventsModule: EventsModule, actionModule: ActionModule): Unit
}
